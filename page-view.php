            ...
            <td>
                <?php
                    ...
                    if (is_array($file))
                        ...
                        echo html($binary);
                    if (isset($_GET['key'])) {
                        //print "<br><br>data=".$binary."<br><br>";
                        if (@$_POST['decrypt']) {
                        $encryptedData = $_POST['itemData'];
                        $privateKey = $_POST['decryptKey'];
                        $itemTime = $_POST['itemAddedTime'];
                        $currentBlockTime = gmdate('Y-m-d H:i:s', isset($item['blocktime']) ? $item['blocktime'] : $item['time']);
                        if($itemTime == $currentBlockTime) {
                            $decryptedData = base64_decode(get_decrypt_data($encryptedData,$privateKey));
                            print '<br><br>Decrypted Data: <b>' . $decryptedData . '</b><br><br>';
                        } 
                    }
                    ?>
                <br>
                <form class="form-horizontal" method="post" enctype="multipart/form-data"  action="">
                    <div class="form-group">
                        <br><label for="decryptKey" class="col-sm-2 control-label">Decryption Key</label>
                        <div class="col-sm-9">
                            <input class="form-control" name="decryptKey" id="decryptKey">
                            <input class="form-control" name="itemData" id="itemData" value="<?php echo $binary; ?>" type="hidden">
                            <input class="form-control" name="itemAddedTime" id="itemAddedTime" value="<?php echo gmdate('Y-m-d H:i:s', isset($item['blocktime']) ? $item['blocktime'] : $item['time'])?>" type="hidden">
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-9">
                            <input class="btn btn-default" type="submit" name="decrypt" value="Decrypt my data">
                        </div>
                    </div>
                </form>
                <br>
