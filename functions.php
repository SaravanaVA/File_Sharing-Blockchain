<?php
	...
	function get_encrypt_data($text,$encryptKey)
	{
		$key  = $encryptKey;
		$data = $text;
		$alg  = MCRYPT_BLOWFISH;
		$mode = MCRYPT_MODE_CBC;
		$iv = base64_decode('67F97/MUUHY=');
		$encrypted_data = mcrypt_encrypt($alg, $key, $data, $mode, $iv);		
		$encrypted_text = base64_encode($encrypted_data);
		return $encrypted_text;
	}

	function get_decrypt_data($encryptData,$encryptKey)
	{
		...
		$mode = MCRYPT_MODE_CBC;
		$iv = base64_decode('67F97/MUUHY=');
		$decoded = mcrypt_decrypt($alg,$key,base64_decode($encryptedData),$mode,$iv);
		$decrypt_data = base64_encode($decoded);
		return $decrypt_data;
	}
